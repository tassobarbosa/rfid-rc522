...
void Search_for_matching_Tag(){ 
 ...
 for(i=0; i <= 3; i++) { 
   ...
   if (GoodTag == 1) { 
      lcd_gotoxy(1,2);
      printf(lcd_putc,"Acesso garantido");
      output_high(pin_d0);
      delay_ms(5000);
      output_low(pin_d0);
   } 
...
{ 
   ...
   while(TRUE) {      
    ...
    lcd_gotoxy(1,1);
    printf(lcd_putc,"Procurando Tag\r\n");      
    delay_ms(300);
     if (FoundTag == MI_OK) {  
        lcd_putc('\f'); 
        lcd_gotoxy(1,1);
        printf(lcd_putc,"Tag encontrada"); 
        delay_ms(1000); 
        Get_UID(); 
